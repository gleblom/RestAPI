...
### 🔐 Ошибки при работе со справочниками (Roles / Units)

Ниже перечислены наиболее частые ошибки при создании/обновлении/удалении ролей.

- 400: "Role name is required" — отправлен пустой `name` при создании/обновлении роли. Решение: указать непустое имя.
- 400: "Role level is required" — не указан обязательный `level` (для всех, кроме автоматических ролей). Решение: передать `level`.
- 400: "Role level must be <= 100" — попытка задать уровень больше 100. Решение: использовать значение <= 100.
- 409: "Role already exists" — роль с таким именем уже есть в компании. Решение: выбрать другое имя или удалить существующую роль.
- 409: "Director already exists for this company" — в компании уже есть роль с `level=100`. Решение: удалить или изменить существующего директора перед созданием нового.
- 409: "Role conflict (unique constraint)" — конфликт уникальных ограничений (возможная гонка). Решение: повторить запрос или обработать конфликт на клиенте.
- 404: "Role not found" — роль с указанным `id` не найдена.
- 403: "Access denied" — попытка управлять ролями другой компании или недостаточно прав.
- 409: "Role is already assigned to users" — попытка удалить роль, которая назначена пользователям.

Примечание: администратор и делопроизводитель получают `level=90, sort_order=1` автоматически и не допускаются в маршрутах согласования.

### 🛣️ Ошибки при управлении маршрутами и узлами

Дополнительные ошибки, специфичные для добавления узлов/рёбер маршрута:

- 409: "Step index already exists in this route" — попытка добавить узел с `step_index`, который уже используется в маршруте.
- 409: "Approver already exists in this route" — попытка назначить одного и того же пользователя дважды в одном маршруте.
- 404: "Approver not found" — указанный `approver_id` не найден.
- 403: "Approver must belong to the same company as the route" — согласующий принадлежит другой компании.
- 400: "Approver must have a role with level/sort_order" — профиль согласующего не содержит должности с заданными `level`/`sort_order`.
- 400: "This role cannot be assigned to approval steps" — роль системная (администратор/делопроизводитель) и не может быть согласующим.
- 409: "A role with this level and order already exists in the route" — в маршруте уже есть позиция с тем же `level` и `sort_order`.
- 400: "Self-loop is not allowed" — попытка создать ребро из узла в самого себя.
- 409: "Edge already exists" — такое ребро уже присутствует.
- 400: "Route must be strictly sequential (no branching allowed)" — валидация последовательности не пройдена (ветвление).
- 400: "Route must contain at least 2 steps" — маршрут должен содержать минимум 2 этапа для сохранения.
- 400: "Approver order must go from lower to higher (by level then sort_order)" — порядок ролей в следующем этапе не выше предыдущего по `level`/`sort_order`.
- 400: "Route graph contains a cycle or disconnected back-edge" — обнаружен цикл или несвязный обратный рёбер.

Рекомендации при обработке ошибок маршрутов:
- На клиенте показывать пользователю точное сообщение из API (409/400) и подсказку как исправить (например, изменить `step_index` или выбрать другого согласующего).
- Валидировать на клиенте базовые ограничения (`step_index` уникален, минимум 2 этапа) до отправки запроса, чтобы уменьшить количество обратных вызовов.
