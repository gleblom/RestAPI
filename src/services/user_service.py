...
from src.schemas.users import OtpDTO, ProfileDTO
...
async def add_user_service(db: Annotated[AsyncSession, Depends(get_session)], email: str, password: str, phone: str) -> User:
...
async def refresh_access_token_service(
    ...
    if not refresh_token:
        raise Authentication("Invalid refresh token")

    if cast(datetime, refresh_token.expires_at) < datetime.now(UTC):
        await RefreshTokenRepository.revoke_refresh_token(refresh_token, db)
        await db.commit()
        raise Authentication("Refresh token expired")

    if refresh_token.auth_level != "full":
        raise Authentication("Invalid token state")
    ...
    new_refresh_token_value, new_hash = create_token()
...
async def get_users_service(
   ...
   users = await ProfileRepository.get_users_by_company(company_id, db, user_name, units, roles)
   
   return users
    
      
async def create_token_service(email: str, token_type: str, db: Annotated[AsyncSession, Depends(get_session)]) -> str:
...
async def send_recovery_token_service(email: str, db: Annotated[AsyncSession, Depends(get_session)]):
    ...
    try:
        result = resend.Emails.send({
            ...
            "html": f"Пожалуйста, введите токен для сброса пароля: {confirm_token_value} Время действия: 15 минут"
            })
